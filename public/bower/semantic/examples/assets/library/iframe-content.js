...
;(function() {
  ...
  var
    ...
    interval              = 32,
    logging               = false,
    msgID                 = '[iFrameSizer]',  //Must match host pages msg ID
  ...
  function init(){
    ...
    sendSize('init','Init message from host pages');
  }
  ...
  function setupInPageLinks(){
    ...
    function findTarget(location){
      ...
      function jumpToTarget(target){
        ...
        log('Moving to in pages link (#'+hash+') at x: '+jumpPosition.x+' y: '+jumpPosition.y);
      ...
      if (target){
        ...
        log('In pages link (#' + hash + ') not found in iFrame, so sending to parent');
    ...
    function initCheck(){ //check if pages loaded with location hash after init resize
    ...
    function enableInPageLinks(){
      if(Array.prototype.forEach && document.querySelectorAll){
        ...
        warn('In pages linking not fully supported in this browser! (See README.md for IE8 workaround)');
      }
    }
    ...
    if(inPageLinks.enable){
      ...
      log('In pages linking not enabled');
    }
    ...
    function createMutationObserver(){
      var
        ...
        observer = new MutationObserver(function(mutations) {
          ...
          setupInjectElementLoadListeners(mutations); //Deal with WebKit asyncing image loading when tags are injected into the pages
        });
  ...
  function sendMsg(height,width,triggerEvent,msg,targetOrigin){
    ...
    function sendToParent(){
      ...
      log('Sending message to host pages (' + message + ')');
  ...
  function receiver(event) {
    ...
    function resetFromParent(){
      if (!initLock){
        log('Page size reset by host pages');
